...
export function compareRegimes(user: UserTaxProfile) {
  const nonAgriIncome = user.income - (user.agriculturalIncome || 0);

  let oldTaxableIncome = nonAgriIncome;

  if(user.occupation === "Salaried") {
    oldTaxableIncome = oldTaxableIncome - STANDARD_DEDUCTION;
  }

  const deductions = calculateDeductions(user);
  oldTaxableIncome = oldTaxableIncome - deductions;
  ...
  return {
    ...
    recommendation: oldTax < newTax ? "Old Regime is better" : "New Regime is better"
  };
}
