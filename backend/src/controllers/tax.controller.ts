...
export const calculateOldRegimeController = (req: Request, res: Response) => {
  try {
    const user: UserTaxProfile = req.body;

    const deductions = calculateDeductions(user);

    const taxableIncome = user.income - (user.agriculturalIncome || 0) - deductions;

    const tax = calculateTax(Math.max(taxableIncome, 0), "oldRegime");
...
export const calculateNewRegimeController = (
  ...
  try {
    ...
    const tax = calculateTax(Math.max(taxableIncome, 0), "newRegime");
